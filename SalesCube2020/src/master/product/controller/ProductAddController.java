...
//	  double dheight = Double.parseDouble(height);
	...
	try {
			...
			if(result==0){
				...
				String message = "逋ｻ骭ｲ縺ｧ縺阪∪縺帙ｓ縺ｧ縺励◆縲�<br>";
				request.setAttribute("addmodifyError", message);
			}else{

				request.setAttribute("addmodifysuccess", "登録が完了しました<br>");

				request.setAttribute("addmodifysuccsess", "逋ｻ骭ｲ縺悟ｮ御ｺ�縺励∪縺励◆縲�<br>");

			}
		}catch(NumberFormatException e){
			request.setAttribute("addmodifyinterror", "豁｣縺励＞謨ｰ蛟､繧貞�･蜉帙＠縺ｦ縺上□縺輔＞縲�<br>");
			e.printStackTrace();
		}
