...
//	  double dheight = Double.parseDouble(height);
	...
	try {
			...
			if(result==0){
				...
				String message = "ìoò^Ç≈Ç´Ç‹ÇπÇÒÇ≈ÇµÇΩ<br>";
				request.setAttribute("addmodifyError", message);
			}else{
				request.setAttribute("addmodifysuccsess", "ìoò^Ç™äÆóπÇµÇ‹ÇµÇΩ<br>");
			}
		}catch(NumberFormatException e){
			request.setAttribute("addmodifyinterror", "ê≥ÇµÇ¢êîílÇì¸óÕÇµÇƒÇ≠ÇæÇ≥Ç¢<br>");
			e.printStackTrace();
		}
